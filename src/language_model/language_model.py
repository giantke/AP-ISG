...
class LanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        # self.checkpoint = "/public/home/zhangke/rgrg-main/src/full_model/gpt-2-pubmed-medium"
        self.checkpoint = "/mnt/wsl/PHYSICALDRIVE3p1/rgrg-main/src/full_model/gpt-2-pubmed-medium"
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def forward(self,
        ...
        position_embeds = self.wte(position_ids)

        word_hidden_states = inputs_embeds + position_embeds
        ...
        ones_column = torch.ones(attention_mask_size[:-1] + (1,), dtype=torch.int64, device=self.device)
        attention_mask = torch.cat((ones_column, attention_mask), dim=-1)
